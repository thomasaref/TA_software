...
from matplotlib.pyplot import plot, show, xlabel, ylabel, title, pcolormesh, legend
...
Xf=X(freq)    
plot(freq, dB(S11c[294, :]), label="64 ns from pulse start {}".format(time[294]-time[30]))
#plot(freq, dB((1.0-R(freq))*(sin(Xf)/Xf)**2)-36.45)
plot(freq, dB(Ga(X(freq))*2*sqrt(50.0)/(50.0+Y(freq)))/2.0+6-24)
