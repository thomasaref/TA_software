...
def pp(base_name, dtype=None, prefix=None, get_suffix="_Get", set_suffix="_Set", coercer=pass_f):
    ...
    if dtype is None:
        if get_suffix is not None:
            def get_func(self):
                return coercer(self.get_func(base_name+get_suffix, prefix=prefix))
        if set_suffix is not None:
            def set_func(self, value):
                self.set_func(base_name+set_suffix, coercer(value), prefix=prefix)
    else:
        if get_suffix is not None:
            def get_func(self):
                return coercer(self.get_func(base_name+get_suffix, dtype=dtype, prefix=prefix))
        if set_suffix is not None:
            def set_func(self, value):
                self.set_func(base_name+set_suffix, dtype(coercer(value)), prefix=prefix)
