...
file_path="/Users/thomasaref/Dropbox/Current stuff/Logbook/TA210715A58_cooldown1/Data_0926/TA_A58_scb_ref_fluxmapl_3t4p6GHz.hdf5"
...
plt.pcolormesh(pwr, freq, absolute(Magcom[:, :, powind]-mean(Magcom[:, :, powind], axis=1, keepdims=True)))

#plt.pcolormesh(absolute(amax(Magcom[:1400, :, :], axis=0)-amin(Magcom[:1400, :, :], axis=0)))
...
plt.pcolormesh(-detuning(xxfrq)/(2.0*pi), freq,  absolute(Magcom[:1400, :, powind]-mean(Magcom[:1400, :, powind], axis=1, keepdims=True)))
          # aspect="auto", origin="lower",
          # interpolation="none",
          #  extent=[ amin(pwr),amax(pwr), amin(freq),amax(freq)], 
          #      )
