...
from numpy import array, mod, angle, absolute, dtype, log10, shape, zeros, meshgrid, empty, nan, append, arange, shape
...
#fig = plt.figure(figsize=[7.3, 4.2] ) #fig width, height in in
...
print type(fig)
print rcParams
#print dir(matplotlib.tight_layout)
#print plt.style.available

#plt.tight_layout()
with enaml.imports():
    from enaml_MatPlotLib import Main

app = QtApplication()
view = Main(figures=figures)
view.show()
app.start()
...
#
     ...
     def add_img_plot(self, z, xs, ys):
         self.set_data("imagedata", z)
         self.set_data("x", xs)
         self.set_data("y", ys)
         xyf=XYFormat()
         xyf.plotter=self
         xyf.draw_img_plot("arg", "imagedata", "x", "y")
