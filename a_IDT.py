...
class IDT(Base):
    finger_type=Enum("double", "single").tag(desc="'double' for double fingered, 'single' for single fingered.",
    ...
    @observe('a', 'v', 'p', 'double_finger')
    def _update_f0(self, change):
        v=self.v
        p=self.p*1e-6
        m=self.get_map("finger_type")
        lbda0=m*2*p
        f0=v/lbda0/1.0e9
        self.do_update("f0", f0)
    ...
    def _get_Ct(self, change):
        ...
        if self.finger_type=="double":
            Ct=sqrt(2.0)*W*epsinf*Np
        else:
            Ct=W*epsinf*Np
        self.do_update("Ct", Ct)
