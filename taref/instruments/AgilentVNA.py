...
from taref.core.atom_extension import get_tag, set_tag, log_func
...
def VNA_read(instr):
    ...
    return instr.session.System2.ReadString()

def VNA_write(instr, VNA_string, **kwargs):
    """calls VNA WriteString using string formatting by kwargs"""
    instr.session.System2.WriteString(VNA_string.format(**kwargs))

def VNA_ask(instr, VNA_string, **kwargs):
    """calls VNA WriteString followed by VNA ReadString"""
    VNA_write(instr, VNA_string, **kwargs)
    return VNA_read(instr)
...
class NetworkAnalyzer(Instrument):
    ...
    def booter(self, address):
        ...
        self.session=CreateObject()
        self.ch1=self.session.Channels["Channel1"]

    start_freq = Float(10.0e6).tag(high=50.0e9, low=10.0e6, label = 'VNA start frequency', unit = 'Hz',
                                    aka="self.ch1.StartFrequency")
