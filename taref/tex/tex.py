...
class TEX(Operative):
    ...
    tex_list=List()
    input_tex=Unicode().tag(spec="multiline", no_spacer=True)
    output_tex=Unicode().tag(spec="multiline", no_spacer=True)


    def _observe_input_tex(self, change):
        inlist=self.input_tex.split("\n")
        outlist=[line for line in inlist if line_check(line)]
        self.output_tex="\n".join(outlist)
...
#            self.str_list=read_tex(tex_source)
    ...
    def extract_block(self, name):
        return extract_block(name, self.input_tex.split("\n"))
