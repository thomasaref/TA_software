...
from numpy import (sqrt, float64, shape, reshape, linspace, log10, absolute, cos, sin, mean, transpose, amax, amin,
                   array, squeeze)
...
def print_fundamentals():
    ...
    print "fq_max=sqrt(8*Ejmax*Ec)={} Hz".format(sqrt(8.0*Ejmax*Ec)/h/1.0e9)
    print "fq_max_full={} Hz".format(flux_parabola(0.0)/1.0e9)
    print
    print "Qubit center frequency = {} Hz".format(v/(8*aq)/1.0e9)

f0=4.54e9
fq0=2*5.45e9

print f0/fq0
Gact=0.45*Npq*K2*fq0/1.0e9
print Gact
...
def couple_theory():
    ...
    plot(f/1.0e9, Gact*(sin(Npq/2.0*pi*(f-fq0)/fq0)/(Npq/2.0*pi*(f-fq0)/fq0))**2, label="Qubit IDT Np/2")
    plot(f/1.0e9, Gact*(sin(Npq*pi*(f-fq0)/fq0)/(Npq*pi*(f-fq0)/fq0))**2, label="Qubit IDT Np")
