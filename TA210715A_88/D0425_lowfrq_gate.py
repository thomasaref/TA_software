...
s3a4_wg=TA88_Lyzer(filt_start_ind=50, filt_end_ind=85, on_res_ind=260,# VNA_name="RS VNA", #48, 78
...
def magabs_colormesh(self):
    pl=Plotter(fig_width=9.0, fig_height=6.0, name="magabs_{}".format(self.name))
    pl, pf=colormesh(self.frequency/1e9, self.yoko, self.MagdB.transpose(), plotter=pl)
    #pf.set_clim(-0.3, 0.1)
...
def magfilt_cmesh(self, f0=5.35e9, alpha=0.45):
    ...
    pl=Plotter(fig_width=9.0, fig_height=6.0, name="magabs_{}".format(self.name))
    pl, pf=colormesh(self.yoko, fq_vec/1e9, absolute(Magfilt.transpose()-Magfilt[:,0]).transpose(), plotter=pl)


if __name__=="__main__":
    pl=magabs_colormesh(s3a4_wg)
