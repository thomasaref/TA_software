...
def read_data(self):
    with File(self.rd_hdf.file_path, 'r') as f:
        ...
        self.yoko=data[0, 1, :91].astype(float64)
        self.pwr2=data[0, 2, ::91].astype(float64)
        sm=shape(Magvec)[0]
...
if __name__=="__main__":
    ...
    b=TA53_VNA_Two_Tone_Pwr_Lyzer(name="d1113", on_res_ind=59,#read_data=read_data, # VNA_name="RS VNA",
            rd_hdf=TA53_Read(main_file="Data_1114/S3A4_trans_two_tone_pwr_swp_low.hdf5"), #long_test.hdf5"), #
            #fit_indices=[range(48,154+1), range(276, 578+1)],
             desc="Gate to IDT low frequency",
             offset=-0.3,
             read_data=read_data,
    ...
    b.filt.halfwidth=10 #8 #10
    b.fitter.fit_type="refl_lorentzian"
    b.fitter.gamma=0.1 #0.035
    b.flux_axis_type="yoko" #"flux" #"fq" #
    #a.bgsub_type="Complex" #"Abs" #"dB"
    b.end_skip=20
    #a.flux_indices=[range(len(a.yoko)-1)]

    b.save_folder.main_dir=a.name
    ...
    a.filter_type="None"
    pl1=colormesh(absolute(a.MagcomData[:, :, 160, 0]))
    a.frq2_ind=80
    ...
    ifft_plot(b, pl="hannifft_b")

    if 1:
        tstart=time()
        double_filt= array([[[a.filt.fft_filter(a.MagcomData[:,n, m, p]) for n in range(len(a.yoko))] for m in range(len(a.frq2))] for p in range(len(a.pwr2))])#.transpose()
        print time()-tstart
        ...
        tstart=time()
        double_filtb= array([[[b.filt.fft_filter(b.MagcomData[:,n, m, p]) for n in range(len(b.yoko))] for m in range(len(b.frq2))] for p in range(len(b.pwr2))])#.transpose()
        print time()-tstart
        print double_filtb.shape
        double_filtb=swapaxes(double_filtb, 0, 3)
        double_filtb=swapaxes(double_filtb, 1, 2)
        ...
        print a.frequency[80]
        #print a.frq2
        #[colormesh(10*log10(absolute(double_filt[fi, :, :])/absolute(double_filt[245, :, :]))) for fi in range(0, 500, 20)]
        ps1=[colormesh(a.yoko, a.frq2/1e9, absolute(double_filt[75, :, :, i]).transpose(),
                   pl="f{0}p{1}".format(int(a.frequency[75]/1e9*1000), int(a.pwr2[i])), xlabel="yoko (V)", ylabel="Freq 2 (GHz)",
        auto_xlim=False, x_min=2, x_max=2.9, auto_ylim=False, y_min=4, y_max=8) for i in range(len(a.pwr2))] #.show() #a.frq2, a.yoko,
        for p in ps1:
            p.title=p.name#[:-3]
        [colormesh(b.yoko, b.frq2, absolute(double_filtb[75, :, :, i]).transpose()) for i in range(len(b.pwr2))] #.show() #a.frq2, a.yoko,
        a.save_plots(ps1)
        pl2.show()
    ...
    colormesh(a.yoko, a.pwr2, absolute(double_filt[75, :, 111, :]).transpose())#.show()
    colormesh(a.yoko, a.pwr2, absolute(double_filt[75, :, 75, :]).transpose())#.show()
