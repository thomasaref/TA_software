...
def read_data(self):
    with File(self.rd_hdf.file_path, 'r') as f:
        ...
        self.pwr2=data[0, 1, :11].astype(float64)
        self.yoko=data[0, 2, ::11].astype(float64)

        sm=shape(Magvec)[0]
        s=(sm, self.frq2.shape[0], self.pwr2.shape[0], self.yoko.shape[0]-1)
        Magcom=Magvec[:,0, :-14]+1j*Magvec[:,1, :-14]
        ...
        Magcom=swapaxes(Magcom, 1, 3)
        Magcom=swapaxes(Magcom, 2, 3)
        self.MagcomData=squeeze(Magcom)#[:, 2, :]
        self.stop_ind=len(self.yoko)-1-1
        self.filt.N=len(self.frequency)

a=TA53_VNA_Two_Tone_Pwr_Lyzer(name="d1108", on_res_ind=75,#read_data=read_data, # VNA_name="RS VNA",
        rd_hdf=TA53_Read(main_file="Data_1108/S1A4_all_two_tone_pwr_spec.hdf5"), #long_test.hdf5"), #
...
a.filt.center=58 #141 #106 #58 #27 #139 #106 #  #137
a.filt.halfwidth=15
...
a.filter_type="None"
pl1=colormesh(absolute(a.MagcomData[:, :, 0, 0]))
...
if 1:
    double_filt= array([[[a.filt.fft_filter(a.MagcomData[:,n, m, p]) for n in range(len(a.yoko)-1)] for m in range(len(a.frq2))] for p in range(len(a.pwr2))])#.transpose()
    ...
    ps1=[colormesh(a.yoko[:-1], a.frequency[a.end_skip:-a.end_skip],
                  absolute(double_filt[a.end_skip:-a.end_skip, :, fi, i]), pl="f{0:.3f}p{1}.jpg".format(a.frq2[fi]/1e9, a.pwr2[i])) for i in range(11)]
    ps1.append(colormesh(a.yoko[:-1], a.pwr2, absolute(double_filt[457, :, fi,:]).transpose(),
                         pl="f{0:.3f}.jpg".format(a.frq2[fi])))
    ...
    ps2=[colormesh(a.yoko[:-1], a.frequency[a.end_skip:-a.end_skip],
                  absolute(double_filt[a.end_skip:-a.end_skip, :, fi, i]), pl="f{0:.3f}p{1}.jpg".format(a.frq2[fi]/1e9, a.pwr2[i])) for i in range(11)]
    ps2.append(colormesh(a.yoko[:-1], a.pwr2, absolute(double_filt[457, :, fi,:]).transpose(),
                         pl="f{0:.3f}.jpg".format(a.frq2[fi])))
    ...
    ps3=[colormesh(a.yoko[:-1], a.frequency[a.end_skip:-a.end_skip],
                  absolute(double_filt[a.end_skip:-a.end_skip, :, fi, i]), pl="f{0:.3f}p{1}.jpg".format(a.frq2[fi]/1e9, a.pwr2[i])) for i in range(11)]
    ps3.append(colormesh(a.yoko[:-1], a.pwr2, absolute(double_filt[457, :, fi,:]).transpose(),
                         pl="f{0:.3f}.jpg".format(a.frq2[fi])))
