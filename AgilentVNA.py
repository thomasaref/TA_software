...
class AgilentNetworkAnalyzer(COM_Instrument):
    ...
    def main_params(self):
     ...
     #"TCPIP::129.16.115.134::inst0::INSTR"
     #enable SICL in system settings
    address=Unicode("TCPIP::129.16.115.134::5025::SOCKET") .tag(sub=True, no_spacer=True)     
    ...
    def update_trace_plot(self):
        self.trace_plot.plot_dict["trace_mag"].clt.set_xdata(self.freq)
        S21dB=20.0*log10(absolute(self.S21))
        self.trace_plot.plot_dict["trace_mag"].clt.set_ydata(S21dB)
    ...
    def _default_trace_plot(self):
        ...
        print self.freq.shape, self.S21.shape
        tp.line_plot('trace_mag', self.freq, 20.0*log10(absolute(self.S21)))
        return tp
    ...
    def booter(self, address):
        ...
        log_debug(self.VNA.Initialize(self.address, False, False, "")) #'QueryInstrStatus=true'
