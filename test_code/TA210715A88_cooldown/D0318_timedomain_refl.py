...
class Lyzer(TA88_Fund):
    ...
    def MagAbsTime(self):
        ...
        return absolute(mean(self.Magcom[53:72, :, :], axis=0).transpose())#-mean(self.Magcom[:, 99:100, self.pind].transpose(), axis=1, keepdims=True))


    def _default_rd_hdf(self):
        return TA88_Read(main_file="Data_0318/S4A4_pulse_fluxswp.hdf5")
...
#
            ...
            tstart=f["Traces"]['TAD1 - Trace_t0dt'][0][0]
            tstep=f["Traces"]['TAD1 - Trace_t0dt'][0][1]
            print shape(Magvec)
            sm=shape(Magvec)[0]
            sy=shape(data)
            s=(sm, sy[0], sy[2])
...
def magabstime_colormesh():
    ...
    b1.colormesh("magabs", a.frequency, a.yoko, a.MagAbsTime)
    #b.line_plot("flux_parabola", c.yoko, c.flux_parabola, color="orange", alpha=0.4)
    #b.set_ylim(4.4e9, 4.5e9)
    b1.xlabel="Time (us)"
    b1.ylabel="Magnitude (abs)"
    b1.title="Reflection vs time"
...
def magabs_cs_fit(pwi=0):
    ...
    p = [200e6,4.5e9, 0.0003, 0.00001, 0.1]
