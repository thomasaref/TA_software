...
if __name__=="__main__":
    ...
    if 0:
        ...
        def ifft_plot(self, plotter):
            plotter.line_plot("ifft_{}".format(self.name), absolute(fft.ifft(self.Magcom[:,self.on_res_ind])), label="On resonance")
            plotter.line_plot("ifft_{}".format(self.name), absolute(fft.ifft(self.Magcom[:,0])), label="Off resonance", color="red")
            plotter.legend()
            plotter.set_xlim(0, 100)
            plotter.mpl_axes.xlabel="Time (#)"
            plotter.mpl_axes.ylabel="Absolute Magnitude"
        d=Plotter(fig_width=6, fig_height=4)
        ifft_plot(s4a1_mp, d)
        ...
        def magabs_colormesh(self, plotter):
            ...
            plotter.colormesh("magdBfiltbgsub_{}".format(self.name), self.flux_over_flux0, self.frequency/1e9, self.MagAbs)
            plotter.set_ylim(min(self.frequency/1e9), max(self.frequency/1e9))
            #plotter.set_xlim(min(self.yoko), max(self.yoko))
            plotter.set_xlim(min(self.flux_over_flux0), max(self.flux_over_flux0))

            plotter.mpl_axes.xlabel="Flux $(\Phi/\Phi_0)$"
            plotter.mpl_axes.ylabel="Frequency (GHz)"
            #plotter.mpl_axes.title="MagdB bg sub fluxmap {}".format(self.name)
        d=Plotter(fig_width=6, fig_height=4)
        magabs_colormesh(s4a1_mp, d)
        #d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transabs_small.png")
        #d.show()

        def magabsfilt_colormesh(self, plotter):
            #MagdB=(self.MagdB.transpose()-mean(self.MagdB[:, 499:500], axis=1)).transpose()
            #flux_over_flux0=(self.yoko-qdt.offset)*qdt.flux_factor
            plotter.colormesh("magdBfiltbgsub_{}".format(self.name), self.flux_over_flux0, self.frequency/1e9, self.MagAbsFilt)
            plotter.set_ylim(min(self.frequency/1e9), max(self.frequency/1e9))
            #plotter.set_xlim(min(self.yoko), max(self.yoko))
            plotter.set_xlim(min(self.flux_over_flux0), max(self.flux_over_flux0))
            ...
            #plotter.mpl_axes.title="MagdB bg sub fluxmap {}".format(self.name)
        d=Plotter(fig_width=6, fig_height=4)
        magabsfilt_colormesh(s4a1_mp, d)
        d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transabsfilt_small.png")
        d.show()
    ...
    if 0:
        ...
        def magabsfilt_colormesh(self, plotter):
            ...
            #plotter.mpl_axes.title="MagdB bg sub fluxmap {}".format(self.name)
        d=Plotter(fig_width=6, fig_height=4)
        magabsfilt_colormesh(ps4a1, d)
        d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "timetransabsfilt_small.png")
        d.show()
        if slow:
    ...
    if 0:
        ...
        d=Plotter(fig_width=6, fig_height=4)
        ifft_plot(s1a1_mp, d)
        d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "refl_ifft.pdf")
        d.show()
    ...
    if 0:
        ...
        def magdBfilt_colormesh(self, plotter):
            plotter.colormesh("magdBfilt_{}".format(self.name), self.flux_over_flux0, self.frequency/1e9, self.MagdBFilt)
            plotter.set_ylim(min(self.frequency/1e9), max(self.frequency/1e9))
            plotter.set_xlim(min(self.flux_over_flux0), max(self.flux_over_flux0))
            plotter.plot_dict["magdBfilt_{}".format(self.name)].mpl.set_clim(-40,-10)
            plotter.mpl_axes.xlabel="Flux $(\Phi/\Phi_0)$"
            plotter.mpl_axes.ylabel="Frequency (GHz)"

        d=Plotter(fig_width=10, fig_height=7)
        magdBfilt_colormesh(s1a4_w, d)
        #d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transdBfilt.png")
        d.show()

        def magdBfiltbgsub_colormesh(self, plotter):
            plotter.colormesh("magdBfiltbgsub_{}".format(self.name), self.flux_over_flux0, self.frequency/1e9, self.MagdBFiltbgsub)
            plotter.set_ylim(min(self.frequency/1e9), max(self.frequency/1e9))
            plotter.set_xlim(min(self.flux_over_flux0), max(self.flux_over_flux0))
            plotter.plot_dict["magdBfiltbgsub_{}".format(self.name)].mpl.set_clim(-7,1)
            plotter.mpl_axes.xlabel="Flux $(\Phi/\Phi_0)$"
            plotter.mpl_axes.ylabel="Frequency (GHz)"
            #plotter.mpl_axes.title="MagdB bg sub fluxmap {}".format(self.name)
        d=Plotter(fig_width=10, fig_height=7)
        magdBfiltbgsub_colormesh(s1a4_w, d)
        #d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transdBfilt_bgsub.png")
        d.show()
        #a2.filt_compare(a2.start_ind, bb2)
        #s1a1_mp.filt_compare("filt_compare_off_res", s1a1_mp.start_ind)
        #s1a1_mp.filt_compare("filt_compare_on_res", s1a1_mp.on_res_ind)
        ...
        def magabs_colormesh(self, plotter):
            ...
            plotter.mpl_axes.ylabel="Frequency (GHz)"
            #plotter.mpl_axes.title="MagdB bg sub fluxmap {}".format(self.name)
        d=Plotter(fig_width=7, fig_height=7)
        magabs_colormesh(s1a4_w, d)
        #d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transabs_raw.png")
        def filt_compare(self, ind, plotter=None, **kwargs):
            flux_over_flux0=(self.yoko-qdt.offset)*qdt.flux_factor
            plotter.line_plot("magabs_{}".format(self.name), self.frequency/1e9, self.MagAbs[:, ind],
                              label="$\Phi/\Phi_0={:.2f}$".format(flux_over_flux0[ind]), color=kwargs.get("color", "blue"))
            plotter.plot_dict["magabs_{}".format(self.name)].mpl.linewidth=0.5
            #plotter.line_plot("magabs_{}".format(self.name), self.frequency, self.MagdBFilt[:, ind], label="MagAbs (filtered)")
        d=Plotter(fig_width=7, fig_height=7)
        filt_compare(s1a4_w, s1a4_w.stop_ind, d)
        filt_compare(s1a4_w, s1a4_w.on_res_ind, d, color="red")
        d.legend()

        #d.savefig("/Users/thomasaref/Dropbox/Current stuff/Linneaus180416/", "transabs_raw_cs.pdf")
